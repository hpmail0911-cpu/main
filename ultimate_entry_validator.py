...
FIX-6  Stop-loss capping: validate_risk_management now caps stop distance at
        prop-firm maximum BEFORE later validation, so valid setups are no
        longer rejected with "STOP TOO WIDE".
FIX-7  Removed triplicate stop_loss / market_condition validation blocks
        in _process_signal().
FIX-8  Fixed broken /positions endpoint (was referencing undefined 'signal').
"""
...
# ==============================================================================
TRADERSPOST_WEBHOOK = "https://webhooks.traderspost.io/trading/webhook/40eea1cf-bea2-4c17-91c5-c420ac82fe4d/c76209cc75f83179acacac7c16ffc8f4"
...
def validate_risk_management(alert_data: Dict) -> Tuple[bool, Dict, str]:
    ...
    instrument = ticker[:3] if len(ticker) >= 3 else ticker
    strategy_params = STRATEGY_RISK_PARAMS.get(strategy, {}).get(instrument, {
        'stop_loss_points': 10, 'take_profit_points': 20,
        'default_position_size': 1, 'max_position_size': 1, 'min_rr_ratio': 1.5
    })
    ...
    if stop_loss is None and entry is not None:
        ...
        sl_dist = atr * 1.5 if atr and atr > 0 else strategy_params['stop_loss_points']
        stop_loss = round(entry - sl_dist, 2) if action == 'LONG' else round(entry + sl_dist, 2)
    ...
    # ── FIX-6: Cap stop at prop-firm maximum ──────────────────────────────
    # ATR-based stops (e.g. MGC ATR=20 → 30pt stop) grossly exceed prop-firm
    # limits (MGC max=1.8pts/$18).  Cap the stop distance and recalculate TP
    # to maintain the target R:R.  This is why every MGC signal was blocked
    # with "STOP TOO WIDE".
    prop_limits = PROP_FIRM_LIMITS.get(instrument)
    min_rr = strategy_params.get('min_rr_ratio', 2.0)

    if prop_limits and entry is not None:
        entry_f = float(entry)
        sl_f = float(stop_loss)
        stop_dist = abs(entry_f - sl_f)
        max_stop = prop_limits['max_stop_pts']
        tick = prop_limits['tick_size']
        target_rr = prop_limits['target_rr']

        if stop_dist > max_stop:
            logger.info(
                f"⚙️ Capping stop: {stop_dist:.2f} pts → {max_stop} pts "
                f"(prop firm limit for {instrument})"
            )
            if action == 'LONG':
                stop_loss = round_to_tick(entry_f - max_stop, tick)
            else:
                stop_loss = round_to_tick(entry_f + max_stop, tick)
            alert_data['stop_loss'] = stop_loss

            tp_dist = max_stop * target_rr
            if action == 'LONG':
                take_profit = round_to_tick(entry_f + tp_dist, tick)
            else:
                take_profit = round_to_tick(entry_f - tp_dist, tick)
            alert_data['take_profit'] = take_profit
            logger.info(
                f"⚙️ Adjusted: SL={stop_loss}, TP={take_profit} "
                f"(R:R={target_rr}:1, max risk=${max_stop * prop_limits['point_value']:.0f})"
            )
    # ──────────────────────────────────────────────────────────────────────

    take_profit = alert_data.get('take_profit')
    if take_profit is None and entry is not None:
        atr = alert_data.get('atr', 0)
        tp_dist = atr * 3.0 if atr and atr > 0 else strategy_params['take_profit_points']
        take_profit = round(entry + tp_dist, 2) if action == 'LONG' else round(entry - tp_dist, 2)
        alert_data['take_profit'] = take_profit
        logger.info(f"➕ Added take profit: {take_profit}")
    elif take_profit is None:
        _tp_pts = strategy_params['take_profit_points']
        take_profit = round(entry + _tp_pts, 2) if action == 'LONG' else round(entry - _tp_pts, 2)
        alert_data['take_profit'] = take_profit
...
def _process_signal():
    try:
        ...
        # 1. Risk Management (SL/TP/position size) — now caps stops at prop firm limits
        ...
        # 4b. Atomic duplicate guard (re-check under lock)
        ...
        # 12. AI Market Analysis (optional)
        if analyzer:
            market_data = {
                'symbol':        symbol,
                'action':        action,
        ...
        traderspost_url = TRADERSPOST_WEBHOOK
        ...
        try:
            response = requests.post(traderspost_url, json=tp_payload, timeout=5)
...
def reset_positions():
    """Quick reset — clears all tracked positions instantly.
       Use when broker has no open trades but validator is still blocking.
...
if __name__ == '__main__':
    ...
    logger.info("   1.  Risk Management (SL/TP/position size + prop firm stop cap)")
    ...
    logger.info("   10. Stop-Loss Safety Check (prop firm limits)")
    logger.info("   11. Market Condition Filter (trending/momentum only)")
    ...
    logger.info("⚡ Prop firm stops: MES=4pt/$20  MNQ=9pt/$18  MGC=1.8pt/$18  MCL=0.15pt/$15  MYM=40pt/$20  M2K=4pt/$20")
    logger.info("="*80)
