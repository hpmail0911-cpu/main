...
FIX-6  Stop-loss CAPPING: ATR-based stops are clamped to the instrument's
        prop-firm maximum *before* R:R is checked.  Trades are no longer
        rejected with "STOP TOO WIDE" — they proceed with a tighter stop
        and a proportional TP (target_rr:1).
FIX-7  Removed triplicated validate_stop_loss / validate_market_conditions
        blocks (were running 3× in the webhook handler).
FIX-8  Fixed /positions endpoint (referenced undefined `signal` variable).
"""
...
def expire_stale_positions() -> int:
    """Remove stale positions per-instrument TTL and previous-day cleanup."""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def validate_whipsaw_protection(alert_data: Dict) -> Tuple[bool, str]:
    ...
    if adx is not None and float(adx) > 0 and float(adx) < adx_min:
        return False, (f"BLOCKED: ADX too low ({float(adx):.1f} < {adx_min}) — "
...
def validate_risk_management(alert_data: Dict) -> Tuple[bool, Dict, str]:
    """Validate and normalise SL / TP / position size.

    FIX-6: After computing the raw stop distance (from the alert payload or
    from ATR), the stop is **capped** at the instrument's prop-firm maximum
    *before* the R:R check runs.  This prevents "STOP TOO WIDE" rejections —
    the trade proceeds with a tighter stop and a proportional TP.
    """
    ...
    # ------------------------------------------------------------------
    # Compute stop-loss if the alert didn't provide one
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Compute take-profit if the alert didn't provide one
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # FIX-6: Cap stop at instrument maximum BEFORE R:R check
    # ------------------------------------------------------------------
    if entry is not None and alert_data.get('stop_loss') is not None:
    ...
    # ------------------------------------------------------------------
    # R:R check (now on potentially capped stop/tp)
    # ------------------------------------------------------------------
    ...
    try:
        ...
        if is_duplicate_signal(symbol, action):
            ...
            return jsonify({'status': 'duplicate',
                            'reason': f'Same signal already processed within {DEDUP_WINDOW_SECONDS}s'}), 200

        validation_results = []

        # 1. Risk management (SL/TP/position size) — now caps stop at max
        ...
        if _MCE_AVAILABLE:
            ...
            validation_results.append(("MCE Condition", True,
                                       f"{_mce_ev.condition} Q{'+' if _mce_q_adj >= 0 else ''}{_mce_q_adj}"))
        else:
            _mce_q_adj = 0
        ...
        # Atomic duplicate guard (race-condition protection)
        ...
        if not is_valid:
            ...
            return jsonify({'status': 'blocked',
                            'reason': f'Setup quality too low: {quality_score_adjusted}/100 (min: {min_score})'}), 200

        # 10. Market condition (trending/momentum only — single call, FIX-7)
...
def list_positions():
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT id, strategy, symbol, action, entry_time, entry_price "
              "FROM open_positions ORDER BY entry_time DESC")
    rows = c.fetchall()
    conn.close()
